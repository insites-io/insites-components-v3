#!/usr/bin/env bash
# Move the `v3/latest` alias to a pinned version: forward (promote a tested publish) or back (roll back).
#
#   scripts/promote.sh 3.4.0
#
# publish.sh no longer touches v3/latest (TW#26796432); this is the only script that does. rollback.sh is
# an alias of this file kept for the muscle memory in the runbooks.
#
...
VERSION="${1:-}"
[[ "$VERSION" =~ ^3\.[0-9]+\.[0-9]+$ ]] || { echo "usage: scripts/promote.sh <3.x.y>"; exit 2; }
